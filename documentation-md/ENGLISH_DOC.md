...
| ------------------------------- | --------- | ---------------------------------- |
| `driver(String)`		 |  `ConnectionCraft` 	  | set driver name (if necessary) 			 |
| `url(String)`			 |  `ConnectionCraft` 	  | set url name (`localhost` by default )   |
| `user(String)`		 |  `ConnectionCraft` 	  | set username (`root` by default) 		 |
| `psk(String)`			 |  `ConnectionCraft` 	  | set password (`blank` by default) 		 |
| `db(String)`			 |  `ConnectionCraft` 	  | set db name  (`null` by default)		 |
| `autocommit(boolean)`  |  `ConnectionCraft` 	  | set autocommit flag (`true` by default)  |
| `port(int)`		 	 |  `ConnectionCraft` 	  | set port number (`3306` by defautl) 	 |
| `connect()`		 	 |  `java.sql.Connection` | connect and return connection 			 |
| `validation()`		 | `String`  			  | return empty string if parameters are ok |
